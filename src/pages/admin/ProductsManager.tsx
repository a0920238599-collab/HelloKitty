        ...
        <div className="flex flex-col sm:flex-row justify-between items-center mb-6 space-y-4 sm:space-y-0">
          <div className="flex flex-col sm:flex-row items-start sm:items-center space-y-2 sm:space-y-0 sm:space-x-4 w-full sm:w-auto">
            <div className="flex space-x-2 bg-gray-100 p-1 rounded-lg">
              {(['all', 'unjudged', 'yes', 'no', 'disputed'] as const).map(tab => (
                <button
                  key={tab}
                ...
                >
                  {tab === 'all' ? '全部' : tab === 'unjudged' ? '未判断' : tab === 'yes' ? '判定为是' : tab === 'no' ? '判定为否' : '存疑'}
                </button>
              ))}
            </div>
                  ...
                  {products.map((product) => (
                    <tr key={product.id}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex flex-col space-y-1">
                          <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full w-fit ${
                            ...
                            product.judgment_status === 'disputed' ? 'bg-purple-100 text-purple-800' :
                            'bg-red-100 text-red-800'
                          }`}>
                            {product.judgment_status === 'unjudged' ? '未判断' : product.judgment_status === 'yes' ? '是' : product.judgment_status === 'disputed' ? '存疑' : '否'}
                          </span>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        {product.judgment_status === 'disputed' && (
                          <div className="flex flex-col items-end space-y-2 mb-2 border-b pb-2">
                            <span className="text-xs text-gray-500">最终裁定:</span>
                            <div className="flex space-x-2">
                              <button onClick={() => handleResolveDisputed(product.id, 'yes')} className="text-green-600 hover:text-green-900 border border-green-200 px-2 py-1 rounded">
                                为是
                              </button>
                              <button onClick={() => handleResolveDisputed(product.id, 'no')} className="text-red-600 hover:text-red-900 border border-red-200 px-2 py-1 rounded">
                                为否
                              </button>
                            </div>
                          </div>
